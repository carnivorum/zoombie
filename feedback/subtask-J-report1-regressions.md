...
Version: **6.1.0**. Suite: **1060 passed** (was 1040 before this change; +20 in
[`tests/test_regressions_report1.py`](../tests/test_regressions_report1.py)).
...
So there is no installer change "to install `mcp`". What the report DID establish is
that **`registered: true` proves only that an entry is in a JSON file.** That gap is
the one real installer defect, and it is fixed below.
...
**Defect 1 is client-side.** The report's own §3.3 mechanism — the client fixes its MCP
tool list when a task STARTS, so a task created before the server connected never
grows the tools, and a window reload does not rebuild that task — is behaviour of the
Zoo Code extension, outside this repository. The toolchain's answer is to say so where
a user will look it up (see the new paragraphs in [`setup.md`](../setup.md:364)) and to
make the install prove the server STARTS (`manifest.mcp.available`). The report's
decisive step 5 ("create a NEW task — do the tools appear?") must be run by the user in
the UI; if a new task still shows nothing, then §3.3 is wrong and the next suspect is
client-side config validation, not this installer.
